...
public class LogrosService {
    ...
    private record Premio(String clave,String nombre,String descripcion,String icono,int precio) {}
    ...
    private static final List<Premio> PREMIOS=List.of(
            new Premio("marco-fuego","Marco Fuego","Un marco ardiente para tu entrenador.","🔥",100),
            new Premio("marco-agua","Marco Agua","Un marco inspirado en el océano.","💧",100),
            new Premio("titulo-tcg","Título Maestro TCG","Presume tu pasión por las cartas.","💎",250),
            new Premio("tema-dorado","Tema dorado","Una apariencia especial para tu perfil.","🌟",400),
            new Premio("confeti-legendario","Confeti legendario","Una celebración de victoria exclusiva.","🎉",600),
            new Premio("marco-campeon","Marco Campeón","La recompensa máxima de la tienda.","👑",1000)
    );
    ...
    public Map<String,Object> resumen(String token){
        ...
        return Map.of("registrado",true,"medallas",jugador.getMedallas(),"completados",completados,"total",LOGROS.size(),"logros",lista,"recompensas",PREMIOS.stream().map(p->mapearPremio(p,compradas.contains(p.clave()))).toList());
    }
    ...
    public Map<String,Object> canjear(String token,String clave){
        ...
        return Map.of("correcto",true,"medallas",jugador.getMedallas(),"recompensa",premio.nombre());
    }
    ...
    private Map<String,Object> mapearPremio(Premio p,boolean comprada){return Map.of("clave",p.clave(),"nombre",p.nombre(),"descripcion",p.descripcion(),"icono",p.icono(),"precio",p.precio(),"comprada",comprada);}
}
